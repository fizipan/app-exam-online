				...
				<div class="app-content main-content">
					<div class="side-app">
						...
						if (isset($_POST["editProfile"])) {
							...
							if ($berhasil > 0) {
								echo "<div class='alert alert-success' role='alert'><strong>Sukses ! </strong>Data Siswa berhasil diubah di database.</div>";
							} else {
							...
							<div class="modal fade" id="exampleModal">
								<div class="modal-dialog">
									...
									<div class="modal-body">
											<?php 
												$id_siswa = $_SESSION["id_siswa"];
												$query = mysqli_query($conn, "SELECT * FROM siswa INNER JOIN kelas ON siswa.id_kelas = kelas.id_kelas WHERE id_siswa = $id_siswa");
												$dataSiswa = mysqli_fetch_assoc($query);
											?>
											<div class="form-group">
												<label for="nisn">NISN</label>
												<input type="number" name="nisn" id="nisn" class="form-control" value="<?= $dataSiswa["nisn"]; ?>">
											</div>
											<div class="form-group">
												<label for="nama">Nama</label>
												<input type="text" name="nama" id="nama" class="form-control" value="<?= $dataSiswa["nama_siswa"]; ?>">
											</div>
											<div class="form-group">
												...
												<select name="id_kelas" class="form-control" id="">
													...
													?>
													<option value="<?php echo $row['id_kelas']; ?>" <?= $row["id_kelas"] == $dataSiswa["id_kelas"] ? 'selected' : ''; ?>><?php echo $row['tingkat_kelas']; ?><?php echo $kode; ?><?php echo $row['kode_kelas']; ?></option>
													<?php } ?>
												</select>
											</div>
											<div class="form-group">
												<label for="alamat">Alamat</label>
												<textarea name="alamat" id="alamat" class="form-control"><?= $dataSiswa["alamat"]; ?></textarea>
											</div>
											<div class="form-group">
												<label for="no_hp">No HP</label>
												<input type="number" name="no_hp" id="no_hp" class="form-control" value="<?= $dataSiswa["no_hp"]; ?>">
											</div>
										</div>
                                ...
                                <div class="col-12 col-md-6">
                                    <div class="card overflow-hidden dash1-card border-0">
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-6">
                                                    <h4 class="mb-4">NISN</h4>
                                                    <h4 class="mb-4">Nama</h4>
                                                    <h4 class="mb-4">Tingkat Kelas</h4>
                                                    <h4 class="mb-4">Jurusan</h4>
                                                    <h4 class="mb-4">Ruang Kelas</h4>
                                                    <h4 class="mb-4">Alamat</h4>
                                                    <h4 class="mb-4">No HP</h4>
												</div>
												<?php 
												$id_siswa = $_SESSION["id_siswa"];
												$query = mysqli_query($conn, "SELECT * FROM siswa INNER JOIN kelas ON siswa.id_kelas = kelas.id_kelas WHERE id_siswa = $id_siswa");
												...
												?>
                                                <div class="col-6">
                                                    <p>: <?php echo $row['nisn'] ?></p>
                                                    <p>: <?php echo $row['nama_siswa'] ?></p>
													<p>: <?php echo $row["tingkat_kelas"] ?></p>
													<p>: <?php echo $row["jurusan"] ?></p>
													<p>: <?php echo $row["ruang_kelas"] ?></p>
