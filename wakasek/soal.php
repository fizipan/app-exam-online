								...
								<div class="card">
									...
									<?php 	
											if(isset($_POST['tambah'])){
												...
												$input  		= "insert into soal (tingkat_kelas, pertanyaan, pilihan1, pilihan2, pilihan3, pilihan4, jawaban, id_guru, id_mapel, status) values ('$tingkat_kelas','$pertanyaan','$pilihan1','$pilihan2','$pilihan3','$pilihan4','$jawaban','". $_SESSION['id_guru'] ."','$id_mapel','DISETUJUI')";
												...
												} else $gagal 	= "<div class='alert alert-danger' role='alert'><strong>Oh snap! </strong>Tambah Data Soal  GAGAL ! Mohon periksa kembali</div>".mysqli_error($conn);
											}
											...
											if(isset($_POST['ubah'])){
												...
												$input  		= "update soal set id_mapel ='" . $id_mapel . "', tingkat_kelas ='" . $tingkat_kelas . "', pertanyaan ='" . $pertanyaan . "', pilihan1 ='" . $pilihan1 . "', pilihan2 ='" . $pilihan2 . "', pilihan3 ='" . $pilihan3 . "', pilihan4 ='" . $pilihan4 . "', jawaban ='" . $jawaban . "' where id_soal ='" . $id_soal . "'";
												$query_input 	= mysqli_query($conn, $input);
												if($query_input) {
												$sukses 		= "<div class='alert alert-success' role='alert'><strong>Sukses ! </strong>Data Soal berhasil dirubah.</div>";
												} else $gagal 	= "<div class='alert alert-danger' role='alert'><strong>Oh snap! </strong>Ubah Data Soal GAGAL ! Mohon periksa kembali</div>".mysqli_error($conn);
											}
									
											if(isset($_POST['setuju'])){
												$id_soal		= $_POST['id_soal'];

												$input  		= "update soal set status ='DISETUJUI' where id_soal ='" . $id_soal . "'";
												$query_input 	= mysqli_query($conn, $input);
												if($query_input) {
												$sukses 		= "<div class='alert alert-success' role='alert'><strong>Sukses ! </strong>Data Soal berhasil disetujui.</div>";
												} else $gagal 	= "<div class='alert alert-danger' role='alert'><strong>Oh snap! </strong>Ubah Data Soal GAGAL ! Mohon periksa kembali</div>".mysqli_error($conn);
											}
											...
											if(isset($_POST['tolak'])){
												...
												$input  		= "update soal set status ='DITOLAK' where id_soal ='" . $id_soal . "'";
												$query_input 	= mysqli_query($conn, $input);
												if($query_input) {
												$sukses 		= "<div class='alert alert-success' role='alert'><strong>Sukses ! </strong>Data Soal berhasil ditolak.</div>";
												} else $gagal 	= "<div class='alert alert-danger' role='alert'><strong>Oh snap! </strong>Ubah Data Soal GAGAL ! Mohon periksa kembali</div>".mysqli_error($conn);
											}
											...
											if(isset($_POST['hapus'])){
												...
												} else $gagal 	= "<div class='alert alert-danger' role='alert'><strong>Oh snap! </strong>Hapus Data Soal GAGAL ! Mohon periksa kembali</div>".mysqli_error($conn);
											}
											...
											<table class="table table-bordered" id="example1">
												<thead>
													<tr>
														...
														<th class="wd-15p border-bottom-0 text-nowrap">Pertanyaan</th>
														...
														<th class="wd-5p border-bottom-0 text-nowrap"></th>
													</tr>
												</thead>
												<tbody>
													<?php
														...
														if (isset($_POST["sortSubmit"])) {
															...
															$query = mysqli_query($conn, "select * from soal inner join mapel on soal.id_mapel=mapel.id_mapel inner join guru on soal.id_guru=guru.id_guru");
														}
														...
														<td>
															<div class="dropdown">
																...
																<div class="dropdown-menu">
																	...
																	<?php if($row['status']=='DIREVIEW'){ ?>
																	<a class="dropdown-item" href="javascript:void(0)" data-target="#setuju<?php echo $no; ?>" data-toggle="modal">SETUJUI</a>
																	<a class="dropdown-item" href="javascript:void(0)" data-target="#tolak<?php echo $no; ?>" data-toggle="modal">TOLAK</a>
																	<?php } ?>
																			...
																			<div class="col">
																				<select name="id_mapel" class="form-control" id="">
																					<?php
																						...
																						$query1 = mysqli_query($conn, "select * from mapel");
																						while ($row1 = mysqli_fetch_array($query1)) {
																					?>
																					<option value="<?php echo $row1['id_mapel']; ?>" <?php if($dataMapel['id_mapel']==$row1['id_mapel']) echo 'selected="selected"'; ?>><?php echo $row1['nama_mapel']; ?></option>
																					<?php } ?>
																				</select>
																			</div>
																		</div>
													...
													<div class="modal" id="setuju<?php echo $no; ?>">
														<div class="modal-dialog" role="document">
															<form action="" method="post">
															<div class="modal-content modal-content-demo">
																<div class="modal-header">
																	<h6 class="modal-title">SETUJUI SOAL</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
																</div>
																<div class="modal-body">
																	<h5>Yakin akan menyetujui dan menampilkan soal ini pada saat ujian sekolah berlangsung ?</h5>
																	<input type="hidden" name="id_soal" value="<?php echo $row['id_soal']; ?>">
																</div>
																<div class="modal-footer">
																	<button class="btn btn-success" type="submit" name="setuju">SETUJUI</button> <button class="btn btn-default" data-dismiss="modal" type="button">BATALKAN</button>
																</div>
															</div>
															</form>
														</div>
													</div>
													<div class="modal" id="tolak<?php echo $no; ?>">
														<div class="modal-dialog" role="document">
															<form action="" method="post">
															<div class="modal-content modal-content-demo">
																<div class="modal-header">
																	<h6 class="modal-title">TOLAK SOAL</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
																</div>
																<div class="modal-body">
																	<h5>Yakin akan menolak soal ini dan tidak akan ditampilkan pada saat ujian sekolah berlangsung ?</h5>
																	<input type="hidden" name="id_soal" value="<?php echo $row['id_soal']; ?>">
																</div>
																<div class="modal-footer">
																	<button class="btn btn-secondary" type="submit" name="tolak">TOLAK</button> <button class="btn btn-default" data-dismiss="modal" type="button">BATALKAN</button>
																</div>
															</div>
															</form>
														</div>
